...
const RecentlyEditedWidget = () => {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        const response = await get("/admin/api/dashboard/stats", {
        ...
        if (isMounted) setData({ recentActivities: [] });
      } finally {
  ...
  if (
    ...
    return (
      <Box
        ...
        hasRadius
        shadow="tableShadow"
        style={{ border: "1px solid #eaeaef", width: "100%" }}
      >
              ...
              >
                <Flex direction="column" gap={1.5} style={{ width: "100%" }}>
                  ...
                  <Typography
                    ...
                    style={{ fontSize: "12px", display: "block" }}
                  >
                    {activity.action} by {activity.updatedBy} ·{" "}
                    {activity.timeAgo}
                  </Typography>
                </Flex>
              </Box>
